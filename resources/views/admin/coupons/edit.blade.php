...
@section('content')
    <div class="mb-4">
        <h2 class="mb-1">Sửa mã giảm giá</h2>
        <p class="text-muted mb-0">Cập nhật thông tin mã.</p>
    </div>
    ...
    <div class="admin-card p-4">
        <form method="post" action="{{ route('admin.coupons.update', $coupon) }}">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="discount_percent" class="form-control" value="{{ old('discount_percent', $coupon->discount_percent) }}" min="1" max="100" id="discount_percent">
            </div>
            <div class="mb-3">
                <label class="form-label">Giảm tiền</label>
                <input type="number" name="discount_amount" class="form-control" value="{{ old('discount_amount', $coupon->discount_amount) }}" min="0" step="0.01" id="discount_amount">
            </div>
            <div class="mb-3">
                <label class="form-label">Hết hạn</label>
                <input type="date" name="expired_at" class="form-control" value="{{ old('expired_at', $coupon->expired_at?->format('Y-m-d')) }}">
            </div>
            ...
            <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" name="apply_all" value="1" id="apply_all" @checked($coupon->apply_all)>
